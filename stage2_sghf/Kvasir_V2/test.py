...
def main():
    ...
    eye_prenet = eyenet(num_classes=3,backbone='resnet101',temp=1.0,fpn_out=512,head_mid=256,use_aspp=True)
    eye_prenet_weight_path = "gaze_fpnv2_polyp_best_total.pth"
    ...
    with torch.no_grad():
        _ = vit_b(torch.zeros(1, 3, 224, 224, device=device), torch.zeros(1, 1, 224, 224, device=device))  # , torch.zeros(1, 1, 224, 224, device=device)
    ...
    for epoch in range(epochs):
        ...
        precision_train = precision_score(Label_train, Train_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        precision_val = precision_score(Label_val, Val_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        recall_train = recall_score(Label_train, Train_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        recall_val = recall_score(Label_val, Val_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        f1_train = f1_score(Label_train, Train_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        f1_val = f1_score(Label_val, Val_acc1, labels=[0, 1, 2, 3, 4, 5, 6, 7], average='macro')
        Label_val = np.array(Label_val)
        Val_acc1 = np.array(Val_acc1)

        pred_prob = np.zeros((len(Val_acc1), NUM_CLASSES), dtype=np.float32)
        for i, pred in enumerate(Val_acc1):
            pred_prob[i, int(pred)] = 1.0
        ...
        auc1 = calculate_auc(pred_prob, label_val_one_hot, NUM_CLASSES)
        ...
        cm = confusion_matrix(Label_val, Val_acc1, labels=list(range(NUM_CLASSES)))
        ...
        plt.savefig("confusion_matrix_8cls.png", dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
